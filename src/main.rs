...
use vec3::{ColorF64, ColorU8, Point3, Vec3};
...
fn get_ray_color(ray: &Ray, hittable_list : &HittableList, depth : i32) -> ColorF64 {
    ...
    if let Some(hit_record) = hittable_list.hit(&ray, 0.0, INFINITY) {
        ...
        let mut attenuation : ColorF64 = ColorF64::zero();

        hit_record.material().scatter(&ray, &hit_record, &mut attenuation, &mut scattered);
...
fn main() {
    ...
    let sphere1 = Sphere::new(Point3::new(0.5, 0.0, -1.0), 0.5, Box::new(Metal{albedo: ColorF64::new(1., 1., 1.)}));
    let sphere2 = Sphere::new(Point3::new(-0.5, 0.0, -1.0), 0.5, Box::new(Metal{albedo: ColorF64::new(1., 1., 1.)}));
    
    let mut hittable_list: Vec<Box<dyn Hittable>> = vec![Box::new(sphere1), Box::new(sphere2)];

    hittable_list.push(Box::new(Sphere::new(Point3::new(0.0, -100.5, -1.0), 100.0, Box::new(Lambertian{albedo: ColorF64::new(0.5, 0.5, 0.5)}))));
